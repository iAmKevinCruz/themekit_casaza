...
var boostPFSFilterConfig = {
	general: {
        ...
        //hala added this to separate the refine by
        separateRefineByFromFilter: true,//$(window).width() < 1008,
        //end hala
...
(function() {
	...
	FilterResult.prototype.afterRender = function(data, eventType) {
        ...
        var totalProduct = data.total_product + ' items';		
		jQ('.boost-pfs-filter-total-product').html(totalProduct);      
	};
  
    FilterClearButton.prototype.bindEvents = function () {
      if (this.$element) {
        this.$element.on('click', this.onClick.bind(this));
      }
      var clearAllCustomButton = jQ('.boost-clear-all-button');
      if(clearAllCustomButton) {
      	clearAllCustomButton.on('click', this.onClearAll.bind(this));
      }
    }
	...
	/* start-boost-custom */
    /* #boost-79618: Apply All button */
    function closeDrawer() {
		jQ('[data-action="close-drawer"]').trigger('click')      
    }
  
    function filterCount() {
    	var filters = Object.keys(Globals.queryParams).filter(function(param) { return param.indexOf('pf_') > -1; } );
      	return filters.reduce(function(sum, curr) { return sum + Globals.queryParams[curr].length; }, 0);
    }  
    ...
    FilterTree.prototype.renderExtraElements = function(){      
      if (this.$element.find(this.selector.applyAllButton).length == 0) {
        ...
        this.applyAllButton.$element.on('click', closeDrawer);
        
        var count = filterCount();
        
//         this.applyAllButton.$element.html(count > 0 ? ('View ' + count + ' Results') : 'View Results');
        this.applyAllButton.$element.html('View Results');
      }   
      ...
      if (this.$element.find(this.selector.clearAllButton).length == 0) {
        ...
        this.clearAllButton.$element.on('click', closeDrawer);
      }     

    };
  
    FilterTree.prototype.beforeRender = function() {
      this.selector.clearAllButtonContainer = '.boost-pfs-filter-mobile-footer';
    } 


    FilterOptionRangeSlider.prototype.afterInit = function() {
      if (Utils.isMobile()) this.requestInstantly = false;
    }

    FilterOptionItemMultiLevelCollections.prototype.afterInit = function() {      
      if (Utils.isMobile()) this.requestInstantly = true;
    }

    FilterOptionItemMultiLevelTag.prototype.afterInit = function() {      
      if (Utils.isMobile()) this.requestInstantly = false;
    }

    FilterOptionItem.prototype.afterInit = function() {         
      if (Utils.isMobile()) this.requestInstantly = false;
	...
	FilterResult.prototype.afterRender = function(data, eventType) {
        ...
        /* #boost-79618: Add Filter Count */
        jQ('.boost-pfs-filter-total-count').remove()

        var count = filterCount();
      
        if (count > 0) {
          jQ('.CollectionToolbar__Item.CollectionToolbar__Item--filter').append('<span class="boost-pfs-filter-total-count">(' + count + ')</span>');
        } 
	};
  
    var onSelectFilter = FilterOptionItem.prototype.onSelectFilter;
	FilterOptionItem.prototype.onSelectFilter = function() {
      onSelectFilter.call(this);
      
      var count = jQ('.boost-pfs-filter-tree-v .boost-pfs-filter-option-item.selected').length;
     //hala added those conditions
      if(count > 0) { 
        //hala added those conditions
    			     jQ('.boost-pfs-filter-apply-all-button').removeAttr('disabled');
                     jQ('.boost-pfs-filter-clear-all').removeAttr('disabled');                    
                    }else {
                       jQ('.boost-pfs-filter-apply-all-button').attr('disabled','disabled');
                       jQ('.boost-pfs-filter-clear-all').attr('disabled','disabled');
                           }}
